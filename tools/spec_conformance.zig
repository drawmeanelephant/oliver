...
test "expectation partition is complete and named divergences are exact" {
    ...
    // Steady state after the full-conformance milestone: 652 supported, 0
    // not-yet — example 201 (an angle destination directly followed by a
    // would-be title, `[foo]: <bar>(baz)`) now conforms — and the former
    // ATX trailing-backslash divergence (example 646) conforms. These pins
    // move only with a reviewed manifest change.
    try std.testing.expectEqual(@as(usize, 652), counts.supported);
    try std.testing.expectEqual(@as(usize, 0), counts.not_yet);
