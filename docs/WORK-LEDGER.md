...
| lead | Knap boundary | record that Knap is not a frontend: docs/KNAP.md, ARCHITECTURE/FEATURE-MATRIX/CAPABILITIES/README/nav, CLEANROOM session 36, `knap-literal` fixture pinning `{{ }}` / `{% %}` / `{# #}` as CommonMark text; `oliver wrap` is not a precedent | n/a | this PR |
