...
| fenced code blocks | implemented | §4.5, per docs/FENCED-CODE.md. Backtick or tilde opener of length ≥3 with ≤3 virtual leading columns; same-marker closer at least as long, ≤3 virtual-column indent, trailing spaces/tabs only. Backtick info strings reject backticks; the complete trimmed info string resolves §2.4 backslash escapes, is retained, and its first word becomes an escaped `language-…` class. Content is literal, removes up to the opener indentation, normalizes every content line ending to `\n`, and closes without backtracking at the containing block/document end. Emits arena-owned `.code_block { content, info }` and deterministic `<pre><code>`. **Scorecard: 29/29**. |
| indented code blocks | implemented | §4.4, per docs/INDENTED-CODE.md. Four virtual indentation columns (spaces or four-column tabs) open an arena-owned literal code leaf; internal tabs and trailing spaces remain literal, provisional blank chunks are retained only between code chunks, and the leaf cannot interrupt paragraphs. **Scorecard: 12/12**. |
...
| entities | deferred | §2.5: named + numeric references; entity table is large (HTML5 entities.json) — a dedicated milestone with a compact lookup strategy and an explicit owned-normalized-text model decision. |
...
| emphasis `_x_` | implemented | → shared `.emphasis` / `<em>`. Same-line boundary, nesting, and opaque-code rules are pinned in docs/TEXTILE-EMPHASIS.md. Both references agree. |
| strong `*x*` | implemented | → shared `.strong` / `<strong>`. Same-line boundary, nesting, and opaque-code rules are pinned in docs/TEXTILE-EMPHASIS.md. Both references agree. |
| bold `**x**` / italic `__x__` | implemented | Doubled Textile phrase modifiers map to the shared `.strong` / `.emphasis` IR (Oliver convergence choice rather than separate `<b>`/`<i>` tags). Both references agree on doubled modifiers. |
...
7. **Markdown tabs.** The spec expands tabs to 4-space tab stops where
   ...
   whitespace (matches the spec's `#→Foo` example).**
