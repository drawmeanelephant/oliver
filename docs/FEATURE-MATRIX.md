...
| block quotes | planned | §5.1. Container block with lazy continuation rules. |
...
| link reference definitions | deferred | §4.7. Required before reference-style links; interacts with the two-pass structure (block pass must collect them before inline pass). |
