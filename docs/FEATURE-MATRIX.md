...
| inline HTML | implemented | §6.6: open/closing tags, comments, processing instructions, declarations, and CDATA sections are discovered across paragraph lines, kept opaque to delimiters/links, and rendered verbatim from their source spans. Invalid shapes remain escaped text; custom tag names are allowed. The renderer policy uses `html.RenderOptions.raw_html` (`allowed` verbatim default, `escaped` HTML-escapes, `rejected` with `error.RawHtmlRejected`; XHTML is always fail-closed) — docs/RAW-HTML.md §3, `--raw-html` on `oliver render`. |
...
| smart typography (extension) | implemented | opt-in `smartypants`: the Textile character-replacement passes (curly quotes by direction, `--` → em dash, space-surrounded `-` → en dash, `...` → ellipsis, digit-adjacent `x` → dimension sign, the parenthesized symbols) in **one shared implementation** (`src/typography.zig`, extracted from Textile with byte-identical output — the Textile wall is the net) and applied to CommonMark plain `.text` nodes in every inline scope (paragraphs, headings, list items, GFM table cells, link display text, callout titles/bodies) with the Textile exemption set (code spans/blocks, autolinks, raw HTML tags, link destinations/titles, image src/alt/title, wikilink targets/labels, escaped `\"`/`\'` — the escape split exempts the byte); the `{...}` character-macro table stays Textile-only (braces literal); borrow-or-copy payloads; off by default (`Options.smartypants`); contract docs/SMARTY.md; issue #67 (v1.0). |
| task lists (extension) | implemented | GFM §6.5 task list items (`Options.task_lists`): a checkbox run (`[ ]` unchecked, `[x]`/`[X]` checked, each followed by a space/tab) at the very start of a list item's first paragraph content becomes a `.task_checkbox` leaf — the paragraph's first inline — rendered as a disabled `<input type="checkbox">` with the rest of the paragraph as the label (emphasis, links, and other extensions work inside it). Recognition is strict: the checkbox must open the item's first block at the content's first bytes; mid-content, later-paragraph, plain-paragraph, and end-of-content checkboxes stay literal, and the whole feature is off by default (corpus untouched). The `<input>` is a void element routed through the render profile (XML form under `.xhtml`; both profiles agree byte-for-byte); the checkbox contributes no plain text. Contract docs/TASK-LISTS.md; issue #92 (v1.1). |
