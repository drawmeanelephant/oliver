...
| front matter (extension) | planned | YAML (`---`) / TOML (`+++`) front matter sniffed at index 0, stripped before dispatch, and parsed into `ParseResult.metadata` (a documented, bounded Oliver-chosen subset — scalars, quoted strings, lists, indented maps; everything outside the subset stays raw with a structured diagnostic, never faked — the Cooklang boundary rule extended); Cooklang's boundary-only `Frontmatter` converges on the shared machinery; opt-in `ParseOptions.frontmatter` (default off — an index-0 `---` is today a §4.1 thematic break, and `+++` is not yet any construct); contract docs/FRONTMATTER.md written before code; issue #66 (v0.5). |
| callouts (extension) | planned | Obsidian-style callouts/admonitions: a leading `[!type]` immediately after the quote marker on a blockquote's first line (`> [!note] Title`) turns the blockquote into a semantic callout (`<div class="callout callout-<type>">` with an optional `<div class="callout-title">`); case-insensitive type, unknown types render as `note`; the remainder of the first line is the title and the rest of the blockquote is the body (nested containers, lists, code all work through the existing container stack); off by default — a plain blockquote with literal `[!note]` text, so the corpus is untouched (`Options.callouts`); contract docs/CALLOUTS.md; issue #65 (v0.6). |
...
| YAML front matter | implemented (boundary only); parsed metadata planned | `---` fences at the very start of the file; the raw payload is preserved with exact spans as `Frontmatter`; Oliver does not parse arbitrary YAML (no fake subset); a missing closing fence degrades to step text with an `unclosed-frontmatter` warning. The shared frontmatter extension (issue #66, v0.5) converges `tryFrontmatter` onto one sniff/strip pre-pass for all three frontends and adds a parsed `metadata` view beside `raw` (bounded Oliver-chosen subset, out-of-subset payloads stay raw). |
