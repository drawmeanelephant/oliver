...
fn parseParagraphInlines(
    ...
    const para_end = lines[lines.len - 1].content.end;
    try discoverLinks(doc, &items, para_end, defs);
...
fn parseHeadingInlines(
    ...
    try discoverLinks(doc, &items, scan.end, defs);
    try matchInlines(doc, node, items.items);
...
/// Discovers inline and reference links (§6.6) in the item list,
/// restructuring it in place. Walks the list with a bracket stack; on a `]`
/// whose nearest `[` opener is followed by a valid `(...)` inline link, or
/// by a `[...]`/`[]` reference label (full/collapsed), or by nothing
/// (shortcut) with a matching definition, splices the bracket range — plus
/// the items covering the consumed `(...)`/label bytes — into a single
/// `link` item whose children are the link-text items, and forgets every
/// earlier `[` (links cannot contain links, so after a link forms no
/// earlier `[` can ever open). Brackets that never form a link stay in the
/// list and emit as literal text. Because a link inside a bracket range
/// would have killed that range's opener, link children never contain
/// links. Runs before emphasis matching: a link item is opaque to the
/// delimiter stack, and its children are matched separately.
///
/// Reference precedence (§6.6): an inline link (`(...)`), then a full
/// reference (`[text][label]`), then collapsed (`[text][]`), then shortcut
/// (`[text]`). A shortcut is suppressed when the `]` is followed by a `[`
/// or `(` (a would-be label or inline destination, whether or not it
/// parses) — "not followed by `[]` or a link label".
fn discoverLinks(doc: *document.Document, items: *std.ArrayList(InlineItem), para_end: u32, defs: *Definitions) ParseError!void {
    const bytes = doc.src.bytes;
    ...
    while (i < old.len) : (i += 1) {
        ...
        if (item == .bracket and item.bracket.ch == ']') {
            ...
            // 1. Inline link: `(` immediately after the `]`.
            if (close_end < para_end and bytes[close_end] == '(') {
                ...
                // A `(` that fails to parse as an inline link falls
                // through to the reference forms (§6.6 example: `[foo](not
                // a link)` with `[foo]: /url1` → shortcut reference, with
                // `(not a link)` as literal text).
                //
                // fall through to the reference checks below
            }

            // 2. Reference forms: a `[` immediately after the `]` is a full
            // or collapsed reference; nothing else is a shortcut.
            if (close_end < para_end and bytes[close_end] == '[') {
            ...
            // failed inline link does not block the shortcut (§6.6
            // example `[foo](not a link)` with `[foo]: /url1`).
